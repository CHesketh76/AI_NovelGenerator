...
def build_directory_tab(self):
    ...
    load_btn = ctk.CTkButton(self.directory_tab, text="Load Novel_directory.txt", command=self.load_chapter_blueprint, font=("Microsoft YaHei", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")
    save_btn = ctk.CTkButton(self.directory_tab, text="Save Changes", command=self.save_chapter_blueprint, font=("Microsoft YaHei", 12))
...
def load_chapter_blueprint(self):
    ...
    self.log("Loaded content of Novel_directory.txt to the editing area.")
