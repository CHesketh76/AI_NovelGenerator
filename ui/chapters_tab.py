...
def build_chapters_tab(self):
    ...
    prev_btn = ctk.CTkButton(top_frame, text="<< Previous Chapter", command=self.prev_chapter, font=("Microsoft YaHei", 12))
    prev_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    next_btn = ctk.CTkButton(top_frame, text="Next Chapter >>", command=self.next_chapter, font=("Microsoft YaHei", 12))
    ...
    save_btn = ctk.CTkButton(top_frame, text="Save Changes", command=self.save_current_chapter, font=("Microsoft YaHei", 12))
    save_btn.grid(row=0, column=3, padx=5, pady=5, sticky="w")

    refresh_btn = ctk.CTkButton(top_frame, text="Refresh Chapters List", command=self.refresh_chapters_list, font=("Microsoft YaHei", 12))
...
def save_current_chapter(self):
    ...
    self.safe_log(f"Changes to chapter {chapter_number_str} have been saved.")
...
def prev_chapter(self):
    ...
    if idx > 0:
        ...
        messagebox.showinfo("Info", "This is already the first chapter.")
...
def next_chapter(self):
    ...
    if idx < len(self.chapters_list) - 1:
        ...
        messagebox.showinfo("Info", "This is already the last chapter.")
